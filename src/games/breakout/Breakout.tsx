...
useEffect(()=>{const c=canvasRef.current;if(!c)return;const ctx=c.getContext('2d')!;const mm=(e:MouseEvent)=>{const r=c.getBoundingClientRect();const scaleX=W/r.width;s.current.px=Math.max(0,Math.min(W-PW,(e.clientX-r.left)*scaleX-PW/2))};const tm=(e:TouchEvent)=>{const r=c.getBoundingClientRect();const scaleX=W/r.width;s.current.px=Math.max(0,Math.min(W-PW,(e.touches[0].clientX-r.left)*scaleX-PW/2))};c.addEventListener('mousemove',mm);c.addEventListener('touchmove',tm,{passive:true});
const loop=()=>{const st=s.current;ctx.fillStyle='#111827';ctx.fillRect(0,0,W,H);if(st.running){st.bx+=st.dx;st.by+=st.dy;if(st.bx-BR<0||st.bx+BR>W)st.dx=-st.dx;if(st.by-BR<0)st.dy=-st.dy;if(st.by+BR>H){st.running=false;setOver(true)}if(st.by+BR>H-30&&st.by+BR<H-18&&st.bx>st.px&&st.bx<st.px+PW){st.dy=-Math.abs(st.dy);st.dx=((st.bx-st.px)/PW-0.5)*6}for(const b of st.bricks){if(!b.alive)continue;if(st.bx+BR>b.x&&st.bx-BR<b.x+BW&&st.by+BR>b.y&&st.by-BR<b.y+BH){b.alive=false;st.dy=-st.dy;st.score+=10;setScore(st.score)}}if(st.bricks.every(b=>!b.alive)){st.running=false;setWon(true)}}for(const b of st.bricks)if(b.alive){ctx.fillStyle=b.color;ctx.fillRect(b.x,b.y,BW,BH)}ctx.fillStyle='#8b5cf6';ctx.fillRect(st.px,H-30,PW,PH);ctx.fillStyle='#fbbf24';ctx.beginPath();ctx.arc(st.bx,st.by,BR,0,Math.PI*2);ctx.fill();anim.current=requestAnimationFrame(loop)};anim.current=requestAnimationFrame(loop);return()=>{cancelAnimationFrame(anim.current);c.removeEventListener('mousemove',mm);c.removeEventListener('touchmove',tm)}},[]);
useEffect(() => {
    if (over && score > 0) {
  ...
  return(
    <GameLayout title="Breakout" score={score} highScore={highScore} onReset={reset}>
      <div className="flex flex-col items-center justify-center w-full h-full gap-4">
        ...
        <button onClick={reset} className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 rounded-lg">{over||won?'Play Again':'Start'}</button>

...
function createBoard(): Board {
  const t = Array.from({ length: SIZE * SIZE - 1 }, (_, i) => i + 1);
  let a: number[];
  do {
    a = [...t, 0].sort(() => Math.random() - 0.5);
  } while (!isSolvable(a));
  return a;
}

function isSolvable(a: number[]): boolean {
  let inv = 0;
  ...
  // Row from bottom is 0-indexed (bottom row = 0)
...
export default function FifteenPuzzle() {
  ...
  const handleClick = (idx: number) => {
    ...
    const br = Math.floor(bi / SIZE), bc = bi % SIZE;
    if ((Math.abs(r - br) === 1 && c === bc) || (Math.abs(c - bc) === 1 && r === br)) {
      const nb = [...board];
      [nb[idx], nb[bi]] = [nb[bi], nb[idx]];
      setBoard(nb);
      setMoves(m => m + 1);
      if (isWon(nb)) setWon(true);
    }
  };
